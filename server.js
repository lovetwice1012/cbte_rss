...
app.get('/callback', async (req, res) => {
    ...
    connection.query('SELECT * FROM rss WHERE userid = ? AND premium_flag = 0 AND username IS NOT NULL AND webhook IS NOT NULL', [user.id], (err, result) => {
        if (err) {
    ...
    if (premium === '1') {
        ...
        connection.query('SELECT * FROM rss WHERE userid = ? AND premium_flag = 1 AND username IS NULL AND webhook IS NULL AND premium_code = ?', [userid, premium_code], (err, result) => {
            ...
            if (result.length > 0) {
                ...
                connection.query('UPDATE rss SET username = ?, webhook = ? WHERE userid = ?, lastextracted = ? AND premium_flag = 1 AND username IS NULL AND webhook IS NULL AND premium_code = ?', [username, webhook, new Date().getTime(), userid, premium_code], (err, result) => {
                    if (err) {
                        console.error('Error executing query:', err);
                        return;
                    }
        ...
        connection.query('SELECT * FROM rss WHERE userid = ? AND premium_flag = 0 AND username IS NULL AND webhook IS NULL', [userid], (err, result) => {
            ...
            if (result.length > 0) {
                //あればusernameとwebhookを更新
                connection.query('UPDATE rss SET username = ?, webhook = ?, lastextracted = ? WHERE userid = ? AND premium_flag = 0 AND username IS NULL AND webhook IS NULL', [username, webhook, new Date().getTime(), userid], (err, result) => {
                    if (err) {
                        console.error('Error executing query:', err);
                        return;
                    }
